...
export default function LoginPage() {
  ...
  const canSubmit = trimmed.length > 0 && trimmed.length <= 20 && !pending;

  const handleLogin = async () => {
    if (!canSubmit) return;
    setPending(true);
    const data = await registerUser(trimmed);
    ...
    localStorage.setItem("pke_nickname", trimmed);
    setPending(false);
  ...
  return (
    <div className="min-h-full flex flex-col transition-colors">
      ...
      <div className="flex-1 flex flex-col items-center px-6 pt-8">
        <img src={BRAND.logo} alt={BRAND.name} className="w-16 h-16 rounded-card shadow-warm" />
        ...
        <div className="w-full mt-8">
          ...
          <button
            ...
            onClick={handleLogin}
            disabled={!canSubmit}
            className="w-full h-12 mt-4 rounded-button text-section-title border-0 disabled:opacity-40 transition-opacity"
            style={CTA_STYLE}
          >
